...
# New: sweep is optional, not mandatory
REQUIRE_SWEEP = False
...
def backtest(df):
    ...
    for i in range(2 * PIVOT_W + 5, len(d) - 1):
        ...
        if in_pos == 0 and allow_new and trades_today < MAX_TRADES_PER_DAY and cooldown == 0:
            ...
            if last_ph_idx is not None and (i - last_ph_idx) <= MAX_BARS_BETWEEN_PEAKS:
                ...
                if len(prev_piv) > 0:
                    ...
                    if (last_ph_idx - prev_i) >= MIN_BARS_BETWEEN_PEAKS:
                        ...
                        sweep_ok = (row['High'] > max(ph1, ph2)) and (row['Close'] < max(ph1, ph2))
                        if tops_close and row['Close'] < neckline and ((not REQUIRE_SWEEP) or sweep_ok):
                            short_signal = True

            if last_pl_idx is not None and (i - last_pl_idx) <= MAX_BARS_BETWEEN_PEAKS:
                ...
                if len(prev_piv) > 0:
                    ...
                    if (last_pl_idx - prev_i) >= MIN_BARS_BETWEEN_PEAKS:
                        ...
                        sweep_ok = (row['Low'] < min(pl1, pl2)) and (row['Close'] > min(pl1, pl2))
                        if bots_close and row['Close'] > neckline and ((not REQUIRE_SWEEP) or sweep_ok):
                            long_signal = True
...
def main():
    ...
    with open('results_us30_reversal_fixed150_v3_doubletop_5m_nosweep_summary.txt', 'w') as f:
        f.write(f"timestamp: {datetime.now().isoformat(timespec='seconds')}\n")
        f.write(f"symbol_proxy: {SYMBOL}\n")
        f.write(f"interval: {INTERVAL}\n")
        f.write(f"period: {PERIOD}\n")
        f.write(f"require_sweep: {REQUIRE_SWEEP}\n")
        for k, v in m.items():
            f.write(f"{k}: {v}\n")
