...
COMP_LOOKBACK = 6
COMP_RANGE_ATR_MAX = 1.1
COMP_WICKY_THRESHOLD = 0.45

# 4H context gate (soft block of strong opposite flow)
HTF_EMA_FAST = 20
HTF_EMA_SLOW = 50
HTF_STRONG_GAP_PCT = 0.0012
...
def add_4h_context(df_15m):
    htf = df_15m.resample('4h').agg({'Open':'first','High':'max','Low':'min','Close':'last'}).dropna()
    htf['ema_f'] = htf['Close'].ewm(span=HTF_EMA_FAST, adjust=False).mean()
    htf['ema_s'] = htf['Close'].ewm(span=HTF_EMA_SLOW, adjust=False).mean()
    htf['trend_up'] = htf['ema_f'] > htf['ema_s']
    htf['trend_dn'] = htf['ema_f'] < htf['ema_s']
    htf['gap_pct'] = ((htf['ema_f'] - htf['ema_s']) / htf['Close']).abs().fillna(0)

    out = df_15m.join(htf[['trend_up','trend_dn','gap_pct']], how='left')
    out[['trend_up','trend_dn']] = out[['trend_up','trend_dn']].ffill().fillna(False)
    out['gap_pct'] = out['gap_pct'].ffill().fillna(0)
    return out
    ...
    for i in range(max(ATR_LEN + 2, COMP_LOOKBACK + 2), len(d) - 1):
        ...
        if can_seek and mode != 0:
            ...
            if move <= 0:
                ...
                strong_4h = row['gap_pct'] >= HTF_STRONG_GAP_PCT

                if mode == 1:
                    # block long if 4H strongly down
                    if bool(row['trend_dn']) and strong_4h:
                        mode = 0
                    else:
                    ...
                    # block short if 4H strongly up
                    if bool(row['trend_up']) and strong_4h:
                        mode = 0
                    else:
                        retrace_level = impulse_close + move / 3.0
...
def main():
    ...
    with open('results_us30_retrace13_v5_4hgate_summary.txt', 'w') as f:
        f.write(f"timestamp: {datetime.now().isoformat(timespec='seconds')}\n")
        f.write(f"symbol_proxy: {SYMBOL}\n")
        f.write(f"interval: {INTERVAL}\n")
        f.write(f"period: {PERIOD}\n")
        f.write(f"htf_ema_fast: {HTF_EMA_FAST}\n")
        f.write(f"htf_ema_slow: {HTF_EMA_SLOW}\n")
        f.write(f"htf_strong_gap_pct: {HTF_STRONG_GAP_PCT}\n")
        for k, v in m.items():
            f.write(f"{k}: {v}\n")
