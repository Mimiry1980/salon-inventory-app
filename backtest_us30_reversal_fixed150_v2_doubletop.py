...
# Double top/bottom params
PIVOT_W = 2
DT_TOL_PCT = 0.0008      # tops within 0.08%
DB_TOL_PCT = 0.0008
...
def backtest(df):
    ...
    for i in range(2 * PIVOT_W + 5, len(d) - 1):
        ...
        if bool(d.iloc[i]['pivot_high']):
            last_ph_idx = i
            last_ph_price = float(d.iloc[i]['High'])
        if bool(d.iloc[i]['pivot_low']):
            last_pl_idx = i
            last_pl_price = float(d.iloc[i]['Low'])
        ...
        if in_pos == 0 and allow_new and trades_today < MAX_TRADES_PER_DAY and cooldown == 0:
            ...
            if last_ph_idx is not None and (i - last_ph_idx) <= MAX_BARS_BETWEEN_PEAKS:
                # find previous pivot high before last_ph_idx
                ...
                if len(prev_piv) > 0:
                    ...
                    if (last_ph_idx - prev_i) >= MIN_BARS_BETWEEN_PEAKS:
                        ...
                        # trigger when close breaks neckline
...
def main():
    ...
    with open('results_us30_reversal_fixed150_v2_doubletop_summary.txt', 'w') as f:
        ...
        f.write(f"rr: {RR}\n")
        f.write(f"double_top_tol_pct: {DT_TOL_PCT}\n")
        for k, v in m.items():
            f.write(f"{k}: {v}\n")
