...
def sample_params(n=2000):
    ...
    for _ in range(n):
        out.append({
            'pivot_w': int(np.random.choice([2, 3])),
            'rr': float(np.random.choice([1.4, 1.5, 1.6, 1.8])),
            'sl_points': float(np.random.choice([120, 130, 140, 150, 160, 180])),
            'max_trades_day': int(np.random.choice([1, 2, 3])),
            'no_new_last_min': int(np.random.choice([20, 30, 40, 50])),
            'wait_open': int(np.random.choice([10, 15, 20, 25, 30])),
            'dt_tol': float(np.random.choice([0.0006, 0.0008, 0.0010, 0.0012, 0.0015])),
            'db_tol': float(np.random.choice([0.0006, 0.0008, 0.0010, 0.0012, 0.0015])),
            'min_bars': int(np.random.choice([2, 3, 4, 5])),
            'max_bars': int(np.random.choice([20, 24, 30, 40, 50, 60])),
            'cooldown': int(np.random.choice([0, 2, 4, 6])),
        })
    return out
...
def main():
    ...
    r = pd.DataFrame(rows)
    # prioritize profitable + low DD + enough trades
    r['score'] = (
        r['profit_factor'].fillna(0) * 1.5
        + r['monthly_return'] * 8
        + (0.03 - r['max_dd'].abs()).clip(lower=-0.3, upper=0.03) * 6
        + (r['trades'].clip(upper=120) / 120) * 0.3
    )

    top = r.sort_values('score', ascending=False).head(30)
    r.to_csv('results_us30_opt2000_all.csv', index=False)
    top.to_csv('results_us30_opt2000_top30.csv', index=False)

    b = top.iloc[0]
    with open('results_us30_opt2000_summary.txt', 'w') as f:
