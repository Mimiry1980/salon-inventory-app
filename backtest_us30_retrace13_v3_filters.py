...
IMPULSE_ATR_MULT = 1.4       # stricter strong move
ATR_LEN = 14
SL_ATR_BUFFER = 0.2
OPPOSITE_BODY_MIN_ATR = 0.35 # opposite candle body min
...
def backtest(df):
    ...
    for i in range(ATR_LEN + 2, len(d) - 1):
        ...
        # manage setup
        if in_pos == 0 and mode != 0 and session_ok and new_trade_ok and trades_today < MAX_TRADES_PER_DAY and np.isfinite(row['atr']):
            ...
            if move <= 0:
                ...
                if mode == 1:
                    ...
                    if row['Low'] <= retrace_level:
                        retrace_done = True
                    # bearish opposite candle with minimum body size
                    ...
                    if row['High'] >= retrace_level:
                        retrace_done = True
                    # bullish opposite candle with minimum body size
                    if retrace_done and row['Close'] > row['Open'] and opp_body >= row['atr'] * OPPOSITE_BODY_MIN_ATR:
                        opposite_candle_idx = i
                    if opposite_candle_idx is not None:
...
def main():
    ...
    with open('results_us30_retrace13_v3_filters_summary.txt', 'w') as f:
        ...
        f.write(f"no_new_trades_last_min: {NO_NEW_TRADES_LAST_MIN}\n")
        f.write(f"impulse_atr_mult: {IMPULSE_ATR_MULT}\n")
        f.write(f"opposite_body_min_atr: {OPPOSITE_BODY_MIN_ATR}\n")
        for k, v in m.items():
            f.write(f"{k}: {v}\n")
