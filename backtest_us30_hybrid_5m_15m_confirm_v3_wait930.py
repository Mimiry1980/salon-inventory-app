...
WAIT_MIN_AFTER_OPEN = 15  # wait for 9:30 manipulation
...
def backtest(df):
    ...
    for i in range(2 * PIVOT_W + 5, len(d) - 1):
        ...
        open_mins = h * 60 + m
        ny_open = SESSION_START[0] * 60 + SESSION_START[1]
        waited_open = open_mins >= (ny_open + WAIT_MIN_AFTER_OPEN)
        allow_new = in_session and mins_to_close > NO_NEW_TRADES_LAST_MIN and waited_open
...
def main():
    ...
    with open('results_us30_hybrid_5m_15m_confirm_v3_wait930_summary.txt', 'w') as f:
